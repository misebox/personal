# 職務経歴書

2025 年 07 月 08 日 更新

氏名：三瀬　一高（みせ　かずたか）

## 略歴

```
2009 関西でソフトウェアエンジニアのキャリアをスタートする。(25歳)
     3年弱ほど勤めた後、退職。
     (Java, VB.net, C#, C言語, Oracle, MySQL など)

2011 地元の愛媛に帰る。
     愛媛の会社で、ソフトウェアエンジニアとして2年ほど勤務。
     (PHP, Delphi, SQLServer など)

2014 スキルアップできる環境をもとめ、上京。
     SES会社のソフトウェアエンジニアとして3年ほど勤務。
     (PHP, Ruby, MySQL, C++)

2017 フリーランスで案件に参画し始める。
     (Python, Django, FastAPI, Go,
      TypeScript, Node.js, React.js, Vue.js,
      MySQL, PostgreSQL,
      AWS, GCP, など)

2024 今に至る
```

↓（逆時系列順）

---

### 2025/02 - 2025/04: GMOサイバーセキュリティ byイエラエ株式会社

#### 脆弱性管理ツールの運用保守

- 重要バグの調査・レポート・修正
- リファクタリングのための技術検証
- コードの調査

##### チーム編成

- フロント: 1
- フルスタック: 3

### 2024/11 - 2025/01: 株式会社プロテーナム（業務委託）

#### ECサイト運用支援ツール

- ECサイトの運用支援ツールの運用保守
- 既存のバグ修正や機能追加を行う。
- 仕様の整理など

- バックエンド
    - バグ修正
    - API追加
- フロントエンド
    - バグ修正
    - 管理画面追加

##### チーム構成

- フルスタック: 2
- インフラ: 1

```
Python, FastAPI, Typescript, React.js, Next.js,
GCP, Cloud Run, Cloud SQL, Cloud Storage, BigQuery, Cloud Scheduler, Cloud Pub/Sub, Cloud Tasks
terraform
```

#### ECサイトのレポート収集ツール

- API利用では仕様上の制限があったため、スクレイピングで必要なレポートを出力させるツールを開発。
- flatによるデスクトップGUIアプリケーション。
- サイトの仕様変更に対応。

### 2024/04 - 2024/10: グローバルネットワークサービス株式会社（業務委託）

#### RFID タグ管理システム

現場で作業担当者が身につけて使用する「RFIDタグ」を管理するシステムを開発。

- 管理画面から指示を出すと、クライアントプログラムがスキャンし、スキャン結果をバックエンドAPIへ送信する。
- 管理画面では、スキャン結果のデータを取得し、登録・従業員との紐づけ・削除などをする。

※　「RFID をスキャンする機器」と「その結果を送信するクライアントプログラム」は、担当外。

- 管理画面とバックエンドAPIの開発を担当。
    - 画面設計
    - API 設計
    - DB 設計
    - バックエンド
    - フロントエンド(半分担当)

##### チーム構成

- フルスタック: 1
- フロントエンド: 1
- クライアントプログラム: 1

```
Python, FastAPI, Typescript, React.js, Next.js,
Terraform, RDS, CloudFront, AppRunner, S3, EC2,
Figma
```

#### 予約管理システム（PMS） のリプレイス

既に指定されていた要件に基づいて、既存のシステムをリプレイス。

不足している機能を追加し、テストを行い、リリース。

現地での動作確認や、ユーザーのフィードバックを受けて、機能改善を行うなど。

- チーム構成
  - バックエンド: 1
  - フロントエンド: 1

```
Python, Django, Typescript, Vue.js, Nuxt.js,
Terraform, RDS, CloudFront, ECS, S3, EC2(NAT Instance)
```

---

### 2024/03 - 2024/03: プラス株式会社 (業務委託)

- EC サイト統合プロジェクトに参加（モブプロ）
- 店舗での注文受付アプリケーションの機能追加

```
Typescript, React.js
APIGateway, AWS Lambda, DynamoDB, Figma,
Confluence, Slack
```

---

### 2023/07 - 2023/09: 株式会社 JX 通信社 (業務委託)

#### データ分析基盤の開発

- PubSub の push サブスクリプションを処理するバックエンドサービスの実装
- データの集計バッチの実装
- API(REST/GraphQL)の実装

#### チーム構成

- 営業: 1
- リーダー: 1
- スクラムマスター: 1
- フロントエンド: 2
- バックエンド: 4-5

#### 作業環境

```
M2 Mac(BYOD), Gitlab, Notion
Go, echo, SQLBoiler
Google Cloud: Cloud SQL, PubSub, Cloud Run
JIRA、Notion
meet / slack など
```

---

### 2022/12 - 2023/06: Immedio 株式会社 (業務委託)

#### ウェブフォーム連動型の商談獲得自動化システムの開発
- PdM と相談しながら開発・運用保守などの作業を担当
- エンジニア 7〜9 人の中のフルタイムメンバー 2 人と一緒にプロセス改善
- perl --> golang の移植
- テーブル設計, フロント UI 作成, API 作成 など

#### 作業環境

```
M2 Mac(BYOD), Github, Notion, Github Actions
JavaScript, TypeScript, React
Perl, Go 1.20, goqu(Query Builder)
MySQL, Docker
Google Cloud: GCE, CloudSQL
```

---

### 2021/09 - 2022/10: 株式会社ミーカンパニー (業務委託)

#### WEB サイトのリプレイス
- バックエンド、フロントエンド
- アーキテクチャ設計
- DB 設計、パフォーマンスチューニング
- テーブルの差分抽出処理

#### 記事検索 API のリプレイス
- バックエンド、フロントエンド
- アーキテクチャ設計

#### 作業環境

```
M1 Mac, BitBucket
JavaScript, TypeScript, Vue3, vite, vite-plugin-ssr
Python3.9, FastAPI, Pydantic, SQLAlchemy, Alembic
MySQL, Docker
AWS: S3, ECS, OpenSearch, RDS, Lambda, CodeBuild, EventBridge
```

---

### 2021/04 - 2021/08: (休暇)

---

### 2020/10 - 2021/03: 株式会社ビザスク (業務委託)

#### ビジネスマッチング WEB サービスの運用保守

- フロントエンド, サーバーサイド
- 管理画面を利用者に話を聞いて、改善したり機能追加をする
- リファクタリング(レガシーコードを置き換えたり、不要なコードを消すなど)

#### 作業環境

```
github, Mac
Python2.7, Django1.11
JS, Vue, GCP, Docker
```

---

### 2020/07 - 2020/09: 株式会社 Automagi (業務委託)

#### 自然言語 AI を活用した社内の業務を効率化するプラットフォーム開発

- フロントエンド, サーバーサイド
- データ投入 UI の実装, バグ修正など
- テスト実施, テスト手法の改善
- Redis で編集ロックの設計

#### 作業環境

```
JS, Vue, Python3.7, Flask, Redis
Docker, AWS EC2, github
```

---

### 2019/12 - 2020/06 (休暇)

---

### 2019/04 - 2019/11: 株式会社アイリッジ (業務委託)

#### ポイント・クーポンアプリ

- バックエンド, クラウドインフラ構築
- データ連携バッチ開発
- アプリのバックエンドの機能修正
- リファクタリング
- 障害調査、不具合修正
- パフォーマンス調査、高速化

#### 作業環境

```
Python3.7, Bottle, Nginx, Docker, gitlab
(AWS)
IAM, CloudFront, CloudFormation,
LAMBDA, ECS, Fargate, Aurora, DynamoDB, S3
StepFunctions, Kinesis Stream
```

---

### 2018/04 - 2019/02: 株式会社ビザスク (業務委託)

#### ビジネスマッチング WEB サービスの運用保守

- フロントエンド, サーバーサイド
- 施策に合わせた機能追加・修正
- パフォーマンス調査、高速化
- 不具合修正
- リファクタリング

#### 作業環境

```
Python2.7, Django1.5, MySQL
JavaScript, ES2015, Vue.js,
GCP, GAS, Docker, github, Mac など
```

---

### 2017/12 - 2018/03: 株式会社デジタルガレージ (業務委託)

#### 新規データプラットフォームシステム開発

- フロントエンド, サーバーサイド
- WEB API の設計開発
- 管理 WEB フロントエンドの機能追加
- DMP 構築へ向けた情報収集のためクローラ作成
- クローラー開発支援ツールとして Chrome 拡張を開発

#### 作業環境

```
Python3.6, Flask, Alembic, PostgreSQL
JavaScript, ES2015, Vue.js, Vuetify, gulp, pugjs
Docker, github, Mac など
```

---

### 2014/08 - 2017/09: 株式会社テクノプロ (雇用契約)

#### パッケージを使用したワークフローシステム開発

- フロントエンド
- パッケージの拡張機能を使用し、パッケージ自体に不足している機能や画面を実装。
- 帳票入力画面の自動入力やバリデーションを JavaScript で実装。
- 不足している画面を追加実装。
-

#### 作業環境

```
Java，JSP，JavaScript，HTML5，ZurbFoundation6
```

#### ハンディターミナル機器のヘルプデスク

- 組み込み OS
- メーカー提供機器の SDK についての問い合わせに回答する。
- サンプルコード作成, メール回答, 問合せ案件のハンドリング

#### 作業環境

```
C++，C＃，VB.NET，Python，Django
```

#### 常駐先企業にて、ニュース配信サイトの運用開発

- フロントエンド, サーバーサイド
- 4,5 人のチーム
- 機能追加, 調査, 障害対応, パフォーマンスチューニング

#### 作業環境

```
Linux, ShellScript, PHP, JS
CodeIgniter, Laravel, Ruby On Rails
MySQL, SubVersion
```

---

### 2013/03 - 2014/04: 株式会社伊予エンジニアリング (雇用契約)

- フロントエンド, サーバーサイド
- Web 対応地理情報システム(Web－GIS)の開発・導入
- 担保評価システムの開発・導入
- ローン案内システムの開発
- 設計,コーディング,テストから顧客サーバールームでの導入作業まで担当。
- 新入社員のフォローなど。

#### 作業環境

```
PHP，Delphi，JS，SQLServer
```

---

### 2011/09 - 2012/06: 株式会社ベルソ (雇用契約)

- HTML/CSS マークアップ, サーバー構築
- WEB サイト管理
- システム監視業務
- コンテンツメンテナンス
- インフラ構築
- WEB デザイン

#### 作業環境

```
Linux，PHP，WordPress，HTML，CSS，CentOS5
```

---

### 2008/11 - 2011/07: ウエストシステム株式会社 (雇用契約)

25 歳の時に基本情報技術者の資格をとり、未経験ながらプログラマーとして転職

- いくつかの企業に派遣される
- デスクトップアプリケーションやサーバーサイド
- 主に既存システムの改修・機能追加・テストを行う

#### 作業環境

```
C＃，Java，JS，HTML，CSS，C言語，MySQL，OracleDB，SQLServer
```
